...
exports.getByCategory = function (req, res, next) {
    QuestionPaper.find().where('category').equals(req.params.category).where('state').equals('PUBLISHED').select('id name').exec(function(err,data){
        if (err) {
            return res.json(500);
        }else{
...
exports.createSamplePaperForTag = function (paperName,category, email,tag,questionIds,callback) {
