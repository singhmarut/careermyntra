...
exports.getSampleQuizzesByCategory = function (req, res, next) {
    ...
    var email;
    getPapers(req,email,true,function(err,data){
        if(err){
            return res.json(500);
        }else{
...
exports.publishPapersByCategory = function (req, res, next) {
    var queryPromise =  Question.find();
    queryPromise = queryPromise.where('category').all(req.body.category);

    var conditions = {category: {$all: [req.body.category]}}
        , update = { $set: { 'state': 'PUBLISHED' }}
        , options = { multi: true,upsert:false };

    QuestionPaper.update(conditions, update, options, function callback (err, numAffected) {
        if (err){
            console.log('unable to update question');
            res.json(500);
        }else{
            console.log('Published papers ' + numAffected);
            res.json(200);
        }
    });
};
