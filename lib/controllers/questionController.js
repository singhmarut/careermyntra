...
/**
 * body {"fieldName": "status", "fieldValue": "PUBLISHED", "tags": ["UPSC2008"]}
 * @param req
 * @param res
 * @param next
 */
exports.bulkUpdateQuestions = function (req, res, next) {

    var fieldName = req.body.fieldName;
    var fieldValue = req.body.fieldValue;

    //{ tags: { $in:  req.body.tags } }
    var conditions = {tags: {$in: [req.body.tags]}}
        , update = { $set: { 'status': fieldValue }}
        , options = { multi: true };

    Question.update({}, update, options, function callback (err, numAffected) {
        if (err){
            console.log('unable to update question');
            res.json(500);
        }else{
            res.json(200);
        }
...
exports.addExtraTags = function (req, res, next) {
    ...
    queryPromise.exec(function(err,data){
        if (!err){
            ...
            search.buildQuestionIndex(questions);
            res.json(200,"Total questions found " + questions.length);
...
/**
 * Publish all questions belonging to these tags
 * @param tags
 * @param callback
 */
exports.publishBulk = function (req, res, next) {

    var tags = req.query.tags;
    var tagsArray = [].concat(tags);

    var query = { qty: { $in: tagsArray } }
        , update = { $inc: { visits: 1 }}
        , options = { multi: true };
    ...
    Question.update(query, { $set: { name: 'jason borne' }}, options, function(err,data){
        if (!err){
            res.json(200);
        }else{
            res.json(500);
...
exports.findByAllTags = function (tags,callback) {
    var queryPromise =  Question.find();
    var questions = [];
    queryPromise = queryPromise.where('status').equals('PUBLISHED').where('tags').all(tags).select('id');
    //Find questions with given tag
    queryPromise.exec(function(err,data){
        var questionIds = [];
