...
exports.createBulkPapersFromTags = function (req, res, next) {
    ...
    _.forEach(papers,function(paper){
        ...
        questionController.findByAllTags(tags,function(err,data){
            ...
            questionPaper.createPaperForTags(paperName,categories, req.session.userInfo.email,paper,data,function(err,data){
                if (err){
            ...
            questionController.findByAllTags(tags,function(err,data){
                ...
                if (papersCount == 0){
                    ...
                    for (var i = 0 ; i < counter; i++){
                        ...
                        questionPaper.createPaperForTags(paperName,categories, req.session.userInfo.email,subject,finalQuestions,function(err,data){
                            if (err){
