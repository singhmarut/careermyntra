...
exports.createBulkPapersFromTags = function (req, res, next) {
    ...
    _.forEach(papers,function(paper){
        ...
        questionController.findByAllTags(tags,function(err,data){
            var paperName = paper;
            questionPaper.createSamplePaperForTag(paperName,'Quizzes', req.session.userInfo.email,paper,data,function(err,data){
                if (err){
            ...
            questionController.findByAllTags(tags,function(err,data){
                ...
                if (papersCount == 0){
                    ...
                    for (var i = 0 ; i < counter; i++){
                        ...
                        idx++;
                        questionPaper.createSamplePaperForTag(paperName,'Quizzes', req.session.userInfo.email,subject,finalQuestions,function(err,data){
                            if (err){
