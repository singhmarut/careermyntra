...
exports.getArticles = function (req, res, next) {
    ...
    getAllArticles(displayAll,function(err,data){
        if (!err){
            res.json(200,data);
        }else{
            console.log("Error while fetching question paper list " + err);
...
function getAllArticles(displayAll,cb){
    if (displayAll){
        Article.where().sort('-createdAt').sort('-publishedAt').exec(function(err,data){
            cb(err,data);
        });
    } else{
        Article.where('status').equals('PUBLISHED').sort('-publishedAt').exec(function(err,data){
            cb(err, data);
        });
    }
};
...
exports.refreshPostIndexes = function (req, res, next) {
